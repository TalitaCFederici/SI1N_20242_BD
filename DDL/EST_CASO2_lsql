...
create schema ESTRS6_II;

#FASE 2: SELECIONAR O NOVO SCHEMA OU DATABASE PARA USO:
use ESTRS6_II;

#CRIANDO A TABELINHA PASSAGEIROS 
...
CONSTRAINT FK_ORIGEM FOREIGN KEY (ORIGEM_VOO) REFERENCES AEROPORTOS(AERO_COD), #DEPENDÊNCIA 
CONSTRAINT FK_DESTINO FOREIGN KEY (DESTINO_VOO) REFERENCES AEROPORTOS(AERO_COD), #DEPENDÊNCIA 
CONSTRAINT FK_AERONAVE FOREIGN KEY (ID_AERONAVES) REFERENCES AERONAVES(COD_AERONAVE) #DEPENDÊNCIA 
);

# Inicio tabela associativas!
CREATE TABLE IF NOT EXISTS EQUIPES(
FNCO_FUNCIONARIOS INT NOT NULL,  
FNCO_VOOS INT NOT NULL,  
PRIMARY KEY(FNCO_FUNCIONARIOS, FNCO_VOOS), 
CONSTRAINT FNCO_FK_FUNCIONARIOS FOREIGN KEY (FNCO_FUNCIONARIOS) REFERENCES FUNCIONARIOS(ID_FUNC),  -- Referência ao funcionário
CONSTRAINT FNCO_FK_VOOS FOREIGN KEY (FNCO_VOOS) REFERENCES VOOS(COD_VOO) 
);
...
FNCO_FUNCIONARIOS INT NOT NULL,
FNCO_VOOS INT NOT NULL,
PRIMARY KEY(COD_VOOS,ID_FUNC),
CONSTRAINT FNCO_FK_FUNCIONARIOS FOREIGN KEY (FNCO_FUNCIONARIOS) REFERENCES FUNCIONARIOS(ID_FUNC),
CONSTRAINT FNCO_FK_VOOS FOREIGN KEY (FNCO_VOOS) REFERENCES VOOS(COD_VOO)
);
